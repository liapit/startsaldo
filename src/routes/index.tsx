...
function Index() {
  ...
  return (
    <main className="bg-background text-foreground">
      ...
      <section className="section-pad bg-card"><div className="section-shell"><h2 className="heading-lg text-center">Was unsere Kunden über die Zusammenarbeit sagen.</h2><div ref={testiRef} className="mt-12 flex snap-x gap-5 overflow-x-auto overflow-y-hidden pb-5" onScroll={(e)=>setTestiScrolled(e.currentTarget.scrollLeft>4)} style={testiScrolled ? undefined : {WebkitMaskImage:"linear-gradient(to right, black calc(100% - 140px), transparent 100%)",maskImage:"linear-gradient(to right, black calc(100% - 140px), transparent 100%)"}}>{[1,2,3,4,5,6].map(x=><Reveal key={x} delay={(x-1)*90} className="min-w-[min(70vw,310px)] snap-start"><article className="h-full rounded-[20px] border border-border bg-background p-8"><p className="text-lg font-medium">Kundenstimme folgt</p><p className="mt-8 text-sm text-muted-foreground">Referenz wird nach Freigabe ergänzt.</p></article></Reveal>)}</div></div></section>
