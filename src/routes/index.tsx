...
function Index() {
  ...
  return (
    <main className="bg-background text-foreground">
      ...
      <section className="section-pad bg-card"><div className="section-shell"><h2 className="heading-lg text-center">Was unsere Kunden über die Zusammenarbeit sagen.</h2><Reveal className="mt-12"><div ref={testiRef} className="flex snap-x gap-5 overflow-x-auto overflow-y-hidden pb-5" onScroll={(e)=>setTestiScrolled(e.currentTarget.scrollLeft>4)} style={testiScrolled ? undefined : {WebkitMaskImage:"linear-gradient(to right, black calc(100% - 140px), transparent 100%)",maskImage:"linear-gradient(to right, black calc(100% - 140px), transparent 100%)"}}>{[1,2,3,4,5,6].map(x=><div key={x} className="min-w-[min(70vw,310px)] snap-start"><article className="h-full rounded-[20px] border border-border bg-background p-8"><p className="text-lg font-medium">Kundenstimme folgt</p><p className="mt-8 text-sm text-muted-foreground">Referenz wird nach Freigabe ergänzt.</p></article></div>)}</div></Reveal></div></section>
